...
The stack-registry capability provides a unified, type-safe registry for detecting and validating technology stack combinations (BuildSystem → Language → Framework). It replaces three independent string-based registries with a single registry using strongly-typed enums, enabling compile-time validation of relationships and automatic chain-based detection.
