...
**When** the test spawns the CLI with `AIPACK_DETECTION_MODE=llm`
...
**Given** a test fixture for a supported language/build system
**When** the test spawns the CLI with `AIPACK_DETECTION_MODE=static`
...
**Given** a test fixture for a supported language/build system
**When** the test spawns the CLI without AIPACK_DETECTION_MODE (defaults to "full")
...
The CLI MUST respect the `AIPACK_DETECTION_MODE` environment variable to control detection mode.

#### Scenario: Environment variable parsing
**Given** the aipack binary is executed
**When** `AIPACK_DETECTION_MODE` is set to "static"
**Then** the binary uses DetectionMode::StaticOnly
**When** `AIPACK_DETECTION_MODE` is set to "llm"
**Then** the binary uses DetectionMode::LLMOnly
**When** `AIPACK_DETECTION_MODE` is set to "full" or not set
**Then** the binary uses DetectionMode::Full (default)

#### Scenario: Static mode without LLM backend
**Given** the binary is executed with `AIPACK_DETECTION_MODE=static`
**When** detection runs
...
**When** running e2e tests with `AIPACK_DETECTION_MODE=static`
...
**Given** e2e tests run with `AIPACK_DETECTION_MODE=llm`
**When** using embedded model via `AIPACK_PROVIDER=embedded`
...
**When** the test runs
**Then** it spawns the aipack binary using Command::new()
**And** passes AIPACK_DETECTION_MODE environment variable
