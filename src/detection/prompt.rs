...
- get_best_practices: Get recommended template for a language + build system combination (REQUIRED - e.g., rust+cargo, javascript+npm)
- submit_detection: Submit your final detection result (ONLY after reading build files and calling get_best_practices)
...
4. Call get_best_practices with the detected language and build system (REQUIRED)
