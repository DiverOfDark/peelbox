use crate::buildkit::llb::LLBBuilder;
use crate::cli::commands::{DetectArgs, FrontendArgs, HealthArgs};
use crate::cli::output::{EnvVarInfo, HealthStatus, OutputFormat, OutputFormatter};
use crate::config::PeelboxConfig;
use crate::detection::service::DetectionService;
use crate::llm::{RecordingLLMClient, RecordingMode};
...
pub async fn handle_detect(args: &DetectArgs, quiet: bool, verbose: bool) -> i32 {
    ...
    let wrap_with_recording =
        |client: Arc<dyn crate::llm::LLMClient>| -> Arc<dyn crate::llm::LLMClient> {
            if std::env::var("PEELBOX_ENABLE_RECORDING").is_ok() {
                ...
                match RecordingLLMClient::new(client.clone(), RecordingMode::Auto, recordings_dir) {
                    Ok(recording_client) => {
                        ...
                        return Arc::new(recording_client) as Arc<dyn crate::llm::LLMClient>;
                    }
                    Err(e) => {
                        warn!(
    ...
    let service = if args.backend.is_some() {
        ...
        .await
        {
            Ok(c) => wrap_with_recording(Arc::new(c) as Arc<dyn crate::llm::LLMClient>),
            Err(e) => {
        ...
        let lazy_client = crate::llm::LazyLLMClient::new(config.clone(), interactive);
        let client = Arc::new(lazy_client) as Arc<dyn crate::llm::LLMClient>;
...
pub async fn handle_frontend(_args: &FrontendArgs) -> i32 {
