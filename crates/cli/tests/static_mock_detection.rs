...
use peelbox_core::fs::MockFileSystem;
use peelbox_llm::LLMClient;
use peelbox_llm::{MockLLMClient, MockResponse};
use peelbox_pipeline::validation::Validator;
use serde_json::json;
...
async fn test_universal_build_validation() {
    ...
    let parsed: peelbox_core::output::schema::UniversalBuild =
...
async fn test_mock_client_error_handling() {
    ...
    client.add_response(MockResponse::error(
        peelbox_core::BackendError::TimeoutError { seconds: 30 },
    ));

    // Try to make a request
    use peelbox_llm::{LLMClient, LLMRequest};
