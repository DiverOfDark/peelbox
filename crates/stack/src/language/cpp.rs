        ...
        match manifest_name {
            "CMakeLists.txt" => {
                ...
                Some(DetectionResult {
                    build_system: crate::BuildSystemId::CMake,
                    confidence,
                })
            }
            "Makefile" => Some(DetectionResult {
                build_system: crate::BuildSystemId::Make,
                confidence: 0.85,
            }),
            "meson.build" => {
                let mut confidence = 0.9;
                ...
                Some(DetectionResult {
                    build_system: crate::BuildSystemId::Meson,
                    confidence,
                })
            }
            _ => None,
...
mod tests {
    ...
    fn test_detect_cmake() {
        ...
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::CMake);
    }
    ...
    fn test_detect_makefile() {
        ...
        assert!(result.is_some());
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Make);
    }
    ...
    fn test_detect_meson() {
        ...
        assert!(result.is_some());
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Meson);
    }
