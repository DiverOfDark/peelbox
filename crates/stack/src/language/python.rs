...
impl LanguageDefinition for PythonLanguage {
    ...
    fn detect(
        ...
        match manifest_name {
            "pyproject.toml" => {
                let mut build_system = crate::BuildSystemId::Pip;
                let mut confidence = 0.85;

                if let Some(content) = manifest_content {
                    if content.contains("[tool.poetry]") {
                        build_system = crate::BuildSystemId::Poetry;
                        confidence = 1.0;
            ...
            "Pipfile" => Some(DetectionResult {
                build_system: crate::BuildSystemId::Pipenv,
                confidence: 1.0,
            }),
            "requirements.txt" => {
                ...
                Some(DetectionResult {
                    build_system: crate::BuildSystemId::Pip,
                    confidence,
                })
            }
...
mod tests {
    ...
    fn test_detect_requirements() {
        ...
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Pip);
    }
    ...
    fn test_detect_pipfile() {
        ...
        assert!(result.is_some());
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Pipenv);
...
"#;
        ...
        assert!(result.is_some());
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Poetry);
        assert_eq!(r.confidence, 1.0);
    }
