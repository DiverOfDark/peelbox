...
impl LanguageDefinition for JavaLanguage {
        ...
        match manifest_name {
            "pom.xml" => {
                ...
                Some(DetectionResult {
                    build_system: crate::BuildSystemId::Maven,
                    confidence,
                })
            }
            "build.gradle" | "build.gradle.kts" => {
                ...
                Some(DetectionResult {
                    build_system: crate::BuildSystemId::Gradle,
                    confidence,
                })
            }
    ...
    fn find_entrypoints(
        ...
        // Normalize empty path to current directory for proper path matching
        let normalized_project_root = if project_root.as_os_str().is_empty() {
            std::path::Path::new(".")
        } else {
            project_root
        };
        ...
        for file_path in file_tree {
            // For proper path matching, we need to ensure we only match files within this specific project
            let is_in_project = if normalized_project_root == std::path::Path::new(".") {
                // For root projects, only include files directly in root (no subdirectories)
                // This prevents matching files from subprojects
                file_path.parent() == Some(std::path::Path::new(""))
                    || file_path.parent() == Some(std::path::Path::new("."))
                    || file_path.parent().is_none()
            } else {
                file_path.starts_with(normalized_project_root)
            };
...
mod tests {
    ...
    fn test_detect_maven() {
        ...
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Maven);
    }
    ...
    fn test_detect_gradle() {
        ...
        assert!(result.is_some());
        let r = result.unwrap();
        assert_eq!(r.build_system, crate::BuildSystemId::Gradle);
    }
